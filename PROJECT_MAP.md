...
### Server Media (SuperAdmin-only)

| File | Kaam | Kahan se use hota hai |
|---|---|---|
| `governance/media_service.py` | **Media ka poora logic.** Listing **database se** hoti hai (chat uploads = `Message` role user, generated media = `Message` role assistant, payment proofs = `Invoice`, logo/favicon = `SiteBranding`), filesystem browse nahi hota — isliye search/pagination SQL mein hai. Filesystem sirf **scan** ke liye chalti hai (`run_scan`: bounded — 100k files / 8s, cache 10 min): totals, orphan candidates, "record hai magar file nahi" list. Har file ka address `(source, pk)` hai, path kabhi user input nahi. Visibility sirf **report** hoti hai (USER-OWNED / PRIVATE (billing) / PUBLIC), badalti nahi. `delete_orphan()` sirf un files ko hatati hai jinhe koi record refer nahi karta — delete ke waqt path safe-join + fresh reference check dobara hota hai | `governance/media_views.py`, `ops_verify` (disk section) |
| `governance/media_views.py` + `templates/governance/media.html` + `_media_icon.html` | Dashboard (totals, disk health NORMAL/WARNING/CRITICAL, filters: search/source/type/extension/owner/size/date, 25/page, Files / Orphan candidates / Missing on disk tabs), **preview** (sirf real png/jpg/gif, PDF, txt/md/csv/json/log — pehle magic bytes check; SVG/HTML/exe kabhi render nahi hote, `415`), **download** (hamesha `attachment` + `application/octet-stream` + `nosniff`, naam sanitized), orphan delete (**typed `DELETE`**, ek file ek baar, bulk route hai hi nahi, form mein path nahi digest jata hai). Sab `SuperAdminRequiredMixin`/`role_required(SUPERADMIN)`. Audit: `media_download`, `media_preview` (5 min mein ek), `media_orphan_deleted`, `media_delete_blocked` — sirf `source:pk`/digest + size/category, file naam ya content kabhi nahi | `governance/urls.py` (`governance:media*`), `templates/base.html` (SuperAdmin nav "Server Media") |
| `governance/test_media_management.py` | 33 tests, 15 required areas (unauthorized, admin denied, pagination, search, protected download, private stays private, traversal, spoofed MIME, referenced file not deletable, orphan detection deletes nothing, audit, no sensitive logging, statistics, storage failure) — throw-away `MEDIA_ROOT`, synthetic files | CI |

**Limited:** referenced files (jin par koi message/invoice/branding record hai) yahan se delete **nahi** hote — record hatane par file khud hat jati hai (`config/files.py`). Storage sirf `default_storage` (local disk); `disk_usage()` sirf local `FileSystemStorage` par kaam karta hai. Scan 100k files/8s par ruk jata hai (`partial` flag dikhata hai). Browser verification: dekhein final report.
...
| Server Media (files dekhna/download/orphan hatana) | `governance/media_service.py` (logic) + `governance/media_views.py` + `templates/governance/media.html`; kabhi path user input se mat banao — `(source, pk)` se lookup |
