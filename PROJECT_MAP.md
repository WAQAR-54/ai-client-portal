...
| `accounts/authz_matrix.py` + `governance/test_authorization_matrix.py` + `governance/authz_expected.json` | **Authorization matrix**: har route (354, Phase 6 ke baad) ko 7 roles (anonymous/user/member/manager/dept-admin/admin/superadmin) se asli backend par hit karta hai. Invariants + golden file: kisi route ka minimum role badle to test fail (jaan-boojh kar badla ho to `AUTHZ_UPDATE=1` se dobara likho aur diff review karo) | CI test job |
