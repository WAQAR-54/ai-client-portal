...
| `governance/views.py::_system_operational_status` + dashboard panel (naya, 2026-09-19) | **SuperAdmin-only** "System status" section existing dashboard mein (dusra dashboard nahi banaya): DB/Redis reachability (`healthz_deep` jaisi cheap check), har connected Provider ka `last_sync_status/last_synced_at/last_sync_error`, aur `django_celery_beat.PeriodicTask` ka `last_run_at/total_run_count/enabled`. Sab "as of last sync/run" label ke sath — LIVE probe nahi (broker `inspect()` ping dashboard ko hang kar sakti hai agar broker down ho). **Abhi nahi hai** (alag project): per-request provider latency/failure/timeout/fallback counters aur Celery retry/duration/last-error — kahin record hi nahi hote | `templates/governance/dashboard.html` |
