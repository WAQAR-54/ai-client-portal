...
| Cloudflare / origin | `accounts/rate_limit.py::client_ip`, `WEB_BIND_ADDRESS` (compose), `deployment/nginx.conf.example` comments | App-side IMPLEMENTED. **Origin port 8000 abhi bhi `0.0.0.0` par** (default badla nahi kyunke live Nginx/firewall repo mein nahi) — `WEB_BIND_ADDRESS=127.0.0.1` aur Cloudflare-only firewall owner ka kaam hai: KNOWN LIMITATION / UNVERIFIED. Agar live Nginx `X-Real-IP` na bheje to purana `CF-Connecting-IP` behaviour (spoof ho sakta hai) |
