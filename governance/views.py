...
class DashboardView(AdminRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        playground_runs_today = _scope_by_user_department(
            self.request, PlaygroundRun.objects.filter(created_at__date=timezone.localdate())
        ).count()
        domain_searches_today = _scope_by_user_department(
            self.request, DomainSearch.objects.filter(created_at__date=timezone.localdate())
        ).count()
        ...
        return super().get_context_data(**kwargs) | {
            ...
            "playground_runs_today": playground_runs_today,
...
def add_user(request):
    ...
    _notify_account_created(new_user)
...
def _notify_account_created(user):
    """Welcome notice for an admin-created account (see add_user). Never
    includes the password itself - it was typed into the create-user form
    by the admin, not generated by us, so the recipient already has to get
    it from them out of band; the email only points at where to log in and
    that "Forgot password?" exists if they need to change it."""
    from notifications.models import NotificationType
    from notifications.notify import notify
...
class BudgetAutomationView(SuperAdminRequiredMixin, TemplateView):
    """Org-wide view of every plan's budget auto-downgrade setting - see
    governance/plans.py::get_budget_automation_status for the runtime check
    this configures. A plan with no monthly_budget_cap has nothing to
    measure spend against, so its row is shown read-only (never toggleable)
    rather than letting an admin turn on a setting that can never fire."""

    template_name = "governance/budget_automation.html"

    def get_context_data(self, **kwargs):
        return super().get_context_data(**kwargs) | {
            "plans": Plan.objects.order_by("-is_default", "name"),
            "fallback_models": ProviderModel.objects.filter(is_enabled=True).select_related("provider"),
        }


@role_required(User.Role.SUPERADMIN)
@require_http_methods(["POST"])
def update_budget_automation(request, plan_id):
    plan = get_object_or_404(Plan, id=plan_id)
    old_value = f"enabled={plan.auto_downgrade_enabled} threshold={plan.auto_downgrade_threshold_pct}"

    # A capless plan has nothing for auto-downgrade to measure against -
    # force it off regardless of what the (disabled, in the template) form
    # control sent, rather than trusting the client not to submit a stray
    # "on".
    plan.auto_downgrade_enabled = bool(plan.monthly_budget_cap) and request.POST.get("auto_downgrade_enabled") == "on"
    threshold = _int_or_none(request.POST.get("auto_downgrade_threshold_pct"))
    if threshold is not None:
        plan.auto_downgrade_threshold_pct = max(1, min(99, threshold))
    fallback_id = request.POST.get("auto_downgrade_fallback_model_id") or None
    plan.auto_downgrade_fallback_model_id = fallback_id
    plan.save(update_fields=["auto_downgrade_enabled", "auto_downgrade_threshold_pct", "auto_downgrade_fallback_model"])

    log_action(
        request.user,
        "plan.budget_automation_update",
        plan,
        old_value=old_value,
...
class RoutingRuleListView(SuperAdminRequiredMixin, ListView):
    model = RoutingRule
    template_name = "governance/routing_rules.html"
    context_object_name = "rules"
    queryset = RoutingRule.objects.select_related("target_model", "target_model__provider").order_by("priority", "id")
...
class RoutingRuleFormView(SuperAdminRequiredMixin, TemplateView):
    template_name = "governance/routing_rule_form.html"

    def get_template_names(self):
        if self.request.headers.get("HX-Request"):
            return ["governance/_routing_rule_form_fields.html"]
        return [self.template_name]

    def get_context_data(self, **kwargs):
        rule = get_object_or_404(RoutingRule, id=kwargs["rule_id"]) if kwargs.get("rule_id") else None
        return super().get_context_data(**kwargs) | {
            "rule": rule,
            "conditions": RoutingRule.Condition.choices,
            "fallback_models": ProviderModel.objects.filter(is_enabled=True).select_related("provider"),
        }

    def post(self, request, rule_id=None):
        rule = get_object_or_404(RoutingRule, id=rule_id) if rule_id else RoutingRule()
        is_new = rule.pk is None

        condition = request.POST.get("condition", "")
        if condition not in RoutingRule.Condition.values:
        ...
        if not target_model_id.isdigit():
            return HttpResponseBadRequest("A target model is required")

        rule.condition = condition
        rule.target_model_id = target_model_id
        rule.priority = _int_or_none(request.POST.get("priority")) or 0
        rule.is_active = request.POST.get("is_active", "on") == "on"
        rule.save()

        log_action(request.user, "routing_rule.create" if is_new else "routing_rule.update", rule, new_value=str(rule))
        if request.headers.get("HX-Request"):
            response = HttpResponse(status=204)
            response["HX-Redirect"] = reverse("governance:routing_rules")
            return response
        return redirect("governance:routing_rules")


@role_required(User.Role.SUPERADMIN)
@require_http_methods(["POST"])
def delete_routing_rule(request, rule_id):
    rule = get_object_or_404(RoutingRule, id=rule_id)
    log_action(request.user, "routing_rule.delete", rule, old_value=str(rule))
    rule.delete()
    if request.headers.get("HX-Request"):
        rules = RoutingRule.objects.select_related("target_model", "target_model__provider").order_by("priority", "id")
        return render(request, "governance/_routing_rules_list.html", {"rules": rules})
    return redirect("governance:routing_rules")


class ComplianceRoutingView(SuperAdminRequiredMixin, ListView):
    """Per-department hard ceiling on which providers a department's
    requests can ever reach - see governance/plans.py::
    region_allowed_provider_model_ids for the enforcement this configures.
    Only one restricted state exists today (EU-only), so the UI is a
    click-to-cycle badge rather than a full form."""

    model = Department
    template_name = "governance/compliance_routing.html"
...
@role_required(User.Role.SUPERADMIN)
@require_http_methods(["POST"])
def update_department_region_restriction(request, department_id):
    department = get_object_or_404(Department, id=department_id)
    region_restriction = request.POST.get("region_restriction", "")
    if region_restriction not in Department.RegionRestriction.values:
        return HttpResponseBadRequest("Invalid region restriction")
    old_value = department.region_restriction
    department.region_restriction = region_restriction
    department.save(update_fields=["region_restriction"])
    log_action(
        request.user,
        "department.region_restriction_update",
        department,
        old_value=old_value,
        new_value=department.region_restriction,
    )
    if request.headers.get("HX-Request"):
        return render(request, "governance/_compliance_routing_row.html", {"department": department})
    return redirect("governance:compliance_routing")


class RetentionProviderApprovalView(SuperAdminRequiredMixin, TemplateView):
    template_name = "governance/retention_provider_approval.html"
    ...
    def get_context_data(self, **kwargs):
        ...
        return super().get_context_data(**kwargs) | {
            "departments": Department.objects.order_by("name"),
            "pending_providers": Provider.objects.filter(
...
@role_required(User.Role.SUPERADMIN)
@require_http_methods(["POST"])
def update_department_retention(request, department_id):
    department = get_object_or_404(Department, id=department_id)
    retention_period = request.POST.get("retention_period", "")
    if retention_period not in Department.RetentionPeriod.values:
        return HttpResponseBadRequest("Invalid retention period")
    old_value = department.retention_period
    department.retention_period = retention_period
    department.save(update_fields=["retention_period"])
    log_action(request.user, "department.retention_update", department, old_value=old_value, new_value=retention_period)
    if request.headers.get("HX-Request"):
...
class DataHandlingView(SuperAdminRequiredMixin, TemplateView):
    template_name = "governance/data_handling.html"

    def get_context_data(self, **kwargs):
        from providers.models import Provider

        rules = {rule.kind: rule for rule in PIIRule.objects.all()}
        return super().get_context_data(**kwargs) | {
            "settings": ComplianceSettings.load(),
            "providers": Provider.objects.order_by("name"),
            "pii_rules": [rules[kind] for kind in _PII_RULE_ORDER if kind in rules],
        }


@role_required(User.Role.SUPERADMIN)
@require_http_methods(["POST"])
def toggle_zero_retention_only(request):
    settings_row = ComplianceSettings.load()
    settings_row.only_zero_retention_models = not settings_row.only_zero_retention_models
    settings_row.save(update_fields=["only_zero_retention_models"])
    log_action(
        request.user,
        "compliance.zero_retention_toggle",
        settings_row,
        new_value=settings_row.only_zero_retention_models,
    )
    return redirect("governance:data_handling")


@role_required(User.Role.SUPERADMIN)
@require_http_methods(["POST"])
def toggle_pii_scanning_enabled(request):
    settings_row = ComplianceSettings.load()
    settings_row.pii_scanning_enabled = not settings_row.pii_scanning_enabled
    settings_row.save(update_fields=["pii_scanning_enabled"])
    log_action(
        request.user, "compliance.pii_scanning_toggle", settings_row, new_value=settings_row.pii_scanning_enabled
    )
    return redirect("governance:data_handling")
...
@role_required(User.Role.SUPERADMIN)
@require_http_methods(["POST"])
def toggle_playground_model(request, model_id):
    """Whether one connected model shows up in the standalone Code
    Playground's model dropdown - see providers.models.ProviderModel.
    is_playground_enabled's docstring for why this is independent of any
    Plan. Only a currently chat-enabled model can be offered here."""
    provider_model = get_object_or_404(ProviderModel, id=model_id, is_enabled=True)
    provider_model.is_playground_enabled = not provider_model.is_playground_enabled
    provider_model.save(update_fields=["is_playground_enabled"])
    log_action(request.user, "playground.model_toggle", provider_model, new_value=provider_model.is_playground_enabled)
    playground_models = ProviderModel.objects.filter(is_enabled=True).select_related("provider")
    if request.headers.get("HX-Request"):
        return render(request, "governance/_playground_model_rows.html", {"playground_models": playground_models})
    return redirect("governance:dashboard")


@role_required(User.Role.SUPERADMIN)
@require_http_methods(["POST"])
def toggle_domain_generator_model(request, model_id):
    """Whether one connected model can be picked in the standalone Domain
    Generator's model dropdown - see providers.models.ProviderModel.
    is_domain_generator_enabled's docstring for why this is independent of
    any Plan. Only a currently chat-enabled model can be offered here."""
    provider_model = get_object_or_404(ProviderModel, id=model_id, is_enabled=True)
    provider_model.is_domain_generator_enabled = not provider_model.is_domain_generator_enabled
    provider_model.save(update_fields=["is_domain_generator_enabled"])
    log_action(
        request.user,
        "domain_generator.model_toggle",
        provider_model,
        new_value=provider_model.is_domain_generator_enabled,
    )
    domain_generator_models = ProviderModel.objects.filter(is_enabled=True).select_related("provider")
    if request.headers.get("HX-Request"):
        return render(
            request,
            "governance/_domain_generator_model_rows.html",
            {"domain_generator_models": domain_generator_models},
        )
    return redirect("governance:dashboard")
...
@role_required(User.Role.SUPERADMIN)
@require_http_methods(["POST"])
def toggle_provider_trains_on_data(request, provider_id):
    from providers.models import Provider

    provider = get_object_or_404(Provider, id=provider_id)
    provider.trains_on_data = not provider.trains_on_data
    provider.save(update_fields=["trains_on_data"])
    log_action(request.user, "provider.trains_on_data_toggle", provider, new_value=provider.trains_on_data)
    if request.headers.get("HX-Request"):
        return render(request, "governance/_zero_retention_cert_row.html", {"provider": provider})
    return redirect("governance:data_handling")


@role_required(User.Role.SUPERADMIN)
@require_http_methods(["POST"])
def update_pii_rule(request, rule_id):
    rule = get_object_or_404(PIIRule, id=rule_id)
    rule.is_enabled = request.POST.get("is_enabled") == "on"
    action = request.POST.get("action", "")
    if action not in PIIRule.Action.values:
        return HttpResponseBadRequest("Invalid action")
    rule.action = action
    rule.save(update_fields=["is_enabled", "action"])
    log_action(request.user, "pii_rule.update", rule, new_value=f"enabled={rule.is_enabled} action={rule.action}")
    if request.headers.get("HX-Request"):
        return render(request, "governance/_pii_rule_row.html", {"rule": rule})
    return redirect("governance:data_handling")


class EmailLogListView(SuperAdminRequiredMixin, TemplateView):
    template_name = "governance/email_logs.html"
...
def update_email_settings(request):
    ...
    if not host or not username:
        return HttpResponseBadRequest("Host and username are required")
    encryption = request.POST.get("encryption", "")
    if encryption not in EmailSettings.Encryption.values:
        return HttpResponseBadRequest("Invalid encryption")

    settings_row = EmailSettings.load()
    port = _int_or_none(request.POST.get("port")) or 587
    from_address = request.POST.get("from_address", "").strip()
    config_changed = (
        settings_row.host != host
        or settings_row.port != port
        or settings_row.encryption != encryption
        or settings_row.username != username
        or settings_row.from_address != from_address
    )

    settings_row.host = host
    settings_row.port = port
    settings_row.encryption = encryption
    settings_row.username = username
    settings_row.from_address = from_address
    # Blank means "leave the saved password alone" - the field is never
    # pre-filled with the real (or a fake placeholder) value, precisely so
    # an unedited submit can't accidentally overwrite it with nothing.
    raw_password = request.POST.get("password", "")
    if raw_password:
        settings_row.set_password(raw_password)
        config_changed = True

    if config_changed and settings_row.status in (EmailSettings.Status.CONNECTED, EmailSettings.Status.FAILED):
        settings_row.status = EmailSettings.Status.STALE
    elif config_changed:
        settings_row.status = EmailSettings.Status.UNVERIFIED

    settings_row.save()
    log_action(request.user, "email_settings.update", settings_row, new_value=f"host={host} username={username}")

    if request.headers.get("HX-Request"):
        return render(request, "governance/_email_config_status.html", {"email_settings": settings_row})
...
def send_test_email(request):
    from notifications.emailing import build_connection, send_via_connection
    from notifications.models import EmailSettings

    host = request.POST.get("host", "").strip()
    username = request.POST.get("username", "").strip()
    to_email = request.POST.get("test_email", "").strip()
    if not host or not username or not to_email:
        return HttpResponseBadRequest("Host, username, and a test recipient are required")
    encryption = request.POST.get("encryption", EmailSettings.Encryption.TLS)
    port = _int_or_none(request.POST.get("port")) or 587
    from_address = request.POST.get("from_address", "").strip() or username

    settings_row = EmailSettings.load()
    # A blank password in the form means "use whatever's already saved" -
    # same rule as update_email_settings, so testing right after opening
    # the modal (before retyping a password) tests the real saved one.
    raw_password = request.POST.get("password", "") or settings_row.get_password()

    connection = build_connection(host, port, username, raw_password, encryption)
    success, error = send_via_connection(
        connection,
        from_address,
        to_email,
    ...
    settings_row.last_test_error = error or ""
    settings_row.save(update_fields=["last_tested_at", "status", "last_test_error"])

    return render(
        request,
        "governance/_email_test_result.html",
        {"success": success, "error": error, "to_email": to_email, "email_settings": settings_row},
    )
